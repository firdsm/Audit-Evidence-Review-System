...
interface BulkExportTemuanButtonProps {
...
export function BulkExportTemuanButton({ institutions }: BulkExportTemuanButtonProps) {
  ...
  const handleToggle = useCallback(
    ...
    [exporting, dropdownOpen, calcPos],
  )
  ...
  const startBulkExport = useCallback(
    async (categoryFilter: string | 'ALL') => {
      ...
      const filtered =
        categoryFilter === 'ALL'
          ...
          : categoryFilter === 'OPD_PRIORITAS'
            ? institutions.filter((i) => !!i.is_priority)
      ...
      try {
        ...
        const zip = new JSZip()
        const folder = zip.folder('Temuan-PEKPPP-2026')
        ...
        for (let i = 0; i < filtered.length; i++) {
          ...
          try {
            const res = await fetch(
              `/api/export-temuan?institutionId=${encodeURIComponent(inst.id)}`,
            )
            ...
            folder?.file(`${fileName}.xlsx`, blob)
          } catch (err) {
        ...
        saveAs(zipBlob, `Temuan-PEKPPP-2026-${categoryLabel}-${today}.zip`)
      } catch (err) {
  ...
  return (
    <>
      ...
      <button
        ...
        title="Export semua temuan per kategori sebagai ZIP"
      >
        <IconSpreadsheetZip className="w-4 h-4 text-indigo-400 opacity-90" />
        Export Semua Temuan
        <svg
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-indigo-500/15 border border-indigo-500/20 flex items-center justify-center shrink-0">
                      <IconSpreadsheetZip className="w-4 h-4 text-indigo-400" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-white">Mengekspor Temuan...</p>
                      <p className="text-xs text-zinc-500 mt-0.5">
                        Mengunduh dan mengemas file Excel ke ZIP
                      </p>
                    </div>
                  </div>
