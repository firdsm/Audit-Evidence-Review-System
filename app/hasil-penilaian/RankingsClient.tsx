...
export default function RankingsClient({
  ...
  const [downloadingScoreId, setDownloadingScoreId] = useState<string | null>(null)
  ...
  // Download Skor Indikator per institusi — simple 2-column Excel
  const handleDownloadSkorIndikator = async (institutionId: string, institutionName: string) => {
    if (downloadingScoreId) return // prevent concurrent downloads
    setDownloadingScoreId(institutionId)
    try {
      const url = `/api/export-skor-indikator?institutionId=${encodeURIComponent(institutionId)}`
      const res = await fetch(url)
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        throw new Error(err.error || `Gagal mengunduh Skor Indikator (${res.status})`)
      }
      const blob = await res.blob()
      const objUrl = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = objUrl
      const safeName = institutionName.replace(/[\/\\:*?"<>|]/g, '').trim()
      a.download = `Skor Indikator - ${safeName}.xlsx`
      a.click()
      window.URL.revokeObjectURL(objUrl)
    } catch (err: any) {
      alert(err.message || 'Gagal mengunduh Skor Indikator')
    } finally {
      setDownloadingScoreId(null)
    }
  }
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-zinc-800/80 text-xs font-bold text-zinc-400 bg-zinc-900/35">
                    ...
                    <th className="px-5 py-3 w-28 text-center"></th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        <td className="px-5 py-2.5 text-center">
                          <div className="inline-flex items-center justify-center gap-1">
                            ...
                            {/* Tombol Skor Indikator — superadmin only */}
                            {isSuperAdmin && (
                            <button
                              onClick={() => handleDownloadSkorIndikator(inst.institutionId, inst.name)}
                              disabled={downloadingScoreId !== null}
                              title={`Unduh Skor Indikator — ${inst.name}`}
                              className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-zinc-800 hover:bg-emerald-600/20 hover:border-emerald-500/40 border border-zinc-700 text-zinc-400 hover:text-emerald-400 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                            >
                              {downloadingScoreId === inst.institutionId ? (
                                <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                                </svg>
                              ) : (
                                /* FileSpreadsheet icon — grid with lines */
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <rect x="3" y="3" width="18" height="18" rx="2" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9h18M3 15h18M9 3v18" />
                                </svg>
                              )}
                            </button>
                            )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-zinc-800/40 space-y-1.5 text-xs">
                      ...
                      <div className="pt-2 border-t border-zinc-800/40 flex gap-2">
                        {/* Tombol Kertas Kerja */}
                        <button
                          onClick={() => handleDownloadKertasKerja(inst.institutionId, inst.name)}
                          disabled={downloadingId !== null}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-lg bg-zinc-800 hover:bg-blue-600/15 border border-zinc-700 hover:border-blue-500/40 text-zinc-400 hover:text-blue-400 text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                        >
                          {downloadingId === inst.institutionId ? (
                            <>
                            ...
                            <>
                              ...
                              <span className="truncate">Kertas Kerja</span>
                            </>
                          )}
                        </button>
                        {/* Tombol Skor Indikator — superadmin only */}
                        ...
                        <button
                          onClick={() => handleDownloadSkorIndikator(inst.institutionId, inst.name)}
                          disabled={downloadingScoreId !== null}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-lg bg-zinc-800 hover:bg-emerald-600/15 border border-zinc-700 hover:border-emerald-500/40 text-zinc-400 hover:text-emerald-400 text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                        >
                          {downloadingScoreId === inst.institutionId ? (
                            <>
                              <svg className="w-3.5 h-3.5 animate-spin shrink-0" fill="none" viewBox="0 0 24 24">
                            ...
                            <>
                              ...
                              <span className="truncate">Skor Indikator</span>
                            </>
                          )}
                        </button>
                        )}
