-- Migration: 20260726010000_update_scoring_function_left_join.sql
-- Drop the old calculation function first
DROP FUNCTION IF EXISTS calculate_institution_scores(UUID);

-- Recreate function calculate_institution_scores using LEFT JOINs to keep unassessed institutions
...
LEFT JOIN  aspects asp
  ON  asp.id = acs.aspect_id
CROSS JOIN active_cfg
LEFT JOIN  aspect_weights aw
  ON  aw.weight_configuration_id = active_cfg.cfg_id
  AND aw.aspect_id               = asp.id
